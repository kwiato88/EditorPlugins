...
Navigator::Navigator(boost::shared_ptr<Plugin::ILocationSetter> p_locationSetter,
...
void Navigator::cutLocationsQueue()
{
    ...
    {
        int l_amoundToCut =
                LOCATIONS_AMOUND_TO_CUT < m_currentLocationIndex
                    ? LOCATIONS_AMOUND_TO_CUT
                    : m_currentLocationIndex;
        m_currentLocationIndex -= l_amoundToCut;
        m_locations.erase(m_locations.begin(), m_locations.begin() + l_amoundToCut);
    }
}
